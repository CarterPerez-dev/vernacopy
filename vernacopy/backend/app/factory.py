...
from clarity.routes import router as clarity_router
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    try:
        synonym_engine.load_wordnet()
    except Exception:
        import structlog
        structlog.get_logger().warning("wordnet_load_failed", msg="WordNet unavailable")

    app.state.synonym_engine = synonym_engine

    yield

    if hasattr(app.state, "synonym_engine"):
        app.state.synonym_engine.shutdown()
...
OPENAPI_TAGS = [
    ...
    {
        "name": "clarity",
        "description": "Word clarity scoring and analysis"
    },
]
...
def create_app() -> FastAPI:
    ...
    app.include_router(clarity_router, prefix = API_PREFIX)
